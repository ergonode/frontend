...
beforeEach(() => {
    cy.apiRequest('POST', 'en/collections').as('POST-REQUEST');
});

Then('I send a {string} request and status code should be {int}', (reqType, status) => {
    getToken();
    sendPostRequest({ reqType, status, urlRegExp: /\/en\/collections$/ });
});
...
Then('I remove {string} element by {string} request', (element, reqType) => {
    removeRequest({ element, reqType, path: 'en/collections' });
});
